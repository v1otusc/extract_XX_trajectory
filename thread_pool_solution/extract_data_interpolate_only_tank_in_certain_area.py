...
2019-08-26 xiaoguo Zhang Only filter Tanker tracks
                         Only filter trackes in certain area
...
SELECTED_VESSEL_TYPES = (80,81,82,83,84,85,86,87,88,89,\
                         1017,1024)                                       #only select tankers
...
class MBR(object):
    ...
    def __init__(self,boundary=[]):
        if(len(boundary) != 4):
            min_value = 999999999999999.0    # specify a big number as min, so that easily to be updated
    ...
    def is_inside(self,longitude, latitude):
        if( (longitude < self.__min_long)  and (longitude > self.__max_long)):
            return False
        elif ( (latitude < self.__min_lat) and (latitude > self.__max_lat)):
            return False
        else:
            return True
...
class vessel_pos_record(object):
    ...
    #......................................................................................................................
    @staticmethod
    def createInstance(line_in_excel, mbr_boudary = None):           
        ...
        if vessel_type in SELECTED_VESSEL_TYPES:   
            ...
            if(mbr_boudary == None):               # do NOT cut map
                return vessel_pos_record(data[MMSI],time_2_second(data[BaseDateTime]),longitude,latitide,run)
            elif(mbr_boudary.is_inside(longitude,latitude)):
            ...
            return None
    #..........................................................................
    def __init__(self,MMSI1,time_second1,long1,lat1,status1,heading =None,others=None):
        self.MMSI = MMSI1
...
class record_tree(object):
    ...
    def push(self,excel_vessel_pos_record_line, mbr = None):   # mbr is an MBR object,  specifying area of which positioning data is valid!
        pos_record = vessel_pos_record.createInstance(excel_vessel_pos_record_line,mbr)
    ...
    def __sort_data__(self):   #sort all positioning data
        notify = process_notify(5)
        debugout('now sorting')
        iall = len(self.data_map)
        i = 0
    ...
    def __interpolate__(pfrom,pto,num):
        points = []
...
def help(argv):
    print('========================================================================')
    print(' Tools to extract tracks of tanker(AND ONLY tanker) to a new csv file')
    print('Usage:')
    print('  '+argv[0]+' source_AIS_file  [minx miny maxx maxy]')
    print('parameters:')
    print('       source_AIS_file - Source AIS file ')
    print('       minx - minx of clip area ')
    print('       miny - miny of clip area ')
    print('       maxx - maxx of clip area ')
    print('       maxy - maxy of clip area ')
    print(' If no minx,miny,maxx,maxy are specified, whole map will be processed')
    print('========================================================================')
    return
...
def main(argv):
    ...
    with open(filename) as file_object:
        ...
        for line in lines[1:]:             #ignore the first line!!!
            ...
            i = i + 1
            if(vessel_hash_table.push(line,user_cut_mbr) == True):
    ...
    str_boundary = ''
    str_boundary = '[OK] The whole original Boundary:'+str(vessel_hash_table.mbr.get_boundary())
    print(str_boundary)
    if(user_cut_mbr != None):
        str_boundary = '[OK] The user-defined area:'+str(user_cut_mbr.get_boundary())
        print(str_boundary)
    ...
    [minlong,minlat,maxlong,maxlat]=[0,0,0,0]
    if(user_cut_mbr == None):
        [minlong,minlat,maxlong,maxlat] = vessel_hash_table.mbr.get_boundary()
    else:
        [minlong,minlat,maxlong,maxlat] = user_cut_mbr.get_boundary()
